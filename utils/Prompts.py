...
BoT_AGENTS_INSTRUCTION = f"""
    ...
    """
CLOTH_RETRIEVE_PROMPT = """The user's query is:{user_input}. From this, please extract the user's requirements and preferences for clothing."
    Please fill in this format and only output the filled content:[clothing type]; [preference]. 
    clothing type is the basic attributes and gender distinction. Other attributes are in preference.
...
BEAUTY_RETRIEVE_PROMPT = """The user's query is:{user_input}. From this, please extract the user's requirements and preferences for beauty product."
    Please fill in this format and only output the filled content:[product type]; [preference]. 
    product type is the basic product type and specification(such as capacity or size). Other attributes are in preference.
    The two parts must be separated by ';'.
    The total length must not exceed 15 words. Separate multiple attributes with ' '.
    Include as much of the key points of user requirements as possible, 
    and the basic attributes of the product are prioritized, followed by the detailed attributes. 
...
The task prompt is: {REACT_PROMPT}\n
...
EVALUATE_PROMPT = """
    ...
    If yes, next you will see a recommendation list of 10 items, all of which point to the same recommendation target, 
    which is {rec_desc} that has been judged above. If {rec_desc} happens to be the main requirement product, 
    then you need to judge whether each product belongs to this type of product. 
    But, if {rec_desc} is not the main requirement, 
    then you should not judge whether these products belongs to the main requirement product, 
    
    because they are another type of product. Instead, you need to make judgement **only** based on whether a product belongs to {rec_desc} itself.
    And if {rec_desc} can meet your general requirement. 
    
    As long as it is, this is enough to prove that it meet your recommendation requirements.
    The recommend list for the recommend target given by the recommendation system is: \"{result_str}\" 
    ...
    output the final rating list in an json format:'{json_format}', score is a pure number.
    """
